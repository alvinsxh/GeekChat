package com.alvin.geekchat.client.ui;
...
import com.alvin.geekchat.client.ClientMain;
import com.alvin.geekchat.client.utils.ResourceUtil;
import com.alvin.geekchat.model.MessagePack;
...
public class ClientUI extends JFrame {
	...
	{
		String iconPath = ResourceUtil.concatResourcesPath("image/app_icon.png");
	...
	ClientUI(String title, User user, BlockingQueue<MessagePack> sendQueue, BlockingQueue<MessagePack> getQueue, String APP_NAME) {
		super(title);
		userID = user.getId();
		userName = new String(user.getName());
	...
	public static ClientUI getClient(User user, User[] userFriends, BlockingQueue<MessagePack> sendQueue, BlockingQueue<MessagePack> getQueue, String APP_NAME) {
		...
		ClientUI ui = new ClientUI(APP_NAME + " - " + user.getName(), user, sendQueue, getQueue, APP_NAME);
	...
	private boolean addFriendButton(User userFriend) {
		friendButtons[friendCount] = new JButton(userFriend.getName());
		friendButtons[friendCount].setPreferredSize(new Dimension(90, 30));
		friendButtons[friendCount].setActionCommand(friendCount + " " + userFriend.getId());
		...
		friendIDs[friendCount] = userFriend.getId();
		friendNames[friendCount] = userFriend.getName();
	...
	private void newMessage(MessagePack pack) {
		if (pack.to.getId() != this.userID) {
			return;
		}
		int fromID = pack.from.getId();
